...
def process_data():
    ...
    return df_list,datelist,time_str
    ...
    if len(df) != 0:
        ...
        def arrange_film_cal(df_cal,field_list,sheet_name,sql = "",sql_field = [],list1= []):
            ...
            if len(field_list) != 1:
                ...
                df_table["场次占比"] = np.round(df_divide(df_table["场次"].astype(float),df_table["总场次"].astype(float)) * 100,2)
                df_table["人次占比"] = np.round(df_divide(df_table["人次"].astype(float),df_table["总人次"].astype(float)) * 100,2)
        ...
        df2["人均票价"] = np.round(df_divide(df2["票房"].astype(float),df2["人次"].astype(float)),2)
        df2["上座率"] = np.round(df_divide(df2["人次"].astype(float),df2["座位数"].astype(float)) *100,2).fillna(0)
        df2["数据日期"] = str(date)
        df2["获取日期"] = str(today)
